...
def run_synchrony_systematic(**kwargs):
    ...
    if kwargs['runsim']:
        ...
        kwargs_list = [{**{
            ...
            for sync_indices in itertools.combinations(range(len(input_sizes_nS)),
                                                       nChoose)
            ]
        with Pool(16, initializer=np.random.seed) as p:
            print(kwargs_list)
            sim_list = list(tqdm(analysis.starstar(p.imap, run_synchrony, kwargs_list),
...
def run_synchrony_experiment(**kwargs):
    ...
    # sync all inputs (50%)
    input_sizes_nS = ([sum([size_nS[x]*N_inputs[x]
    ...
    for kwargs, sim in zip(kwargs_list, sim_list):
        ...
        df_data['sync percent'].append(100*kwargs['N_synced']/N_inputs[kwargs['sync_type']])
...
def run_synchrony_random(**kwargs):
    kwargs_list = [{**{
        ...
        # 'N_synced': 2,
        }, **kwargs} for _ in range(kwargs.get('nreps', 16))]
    kwargs_list.extend([{**{
        'seed': seed,
        'N_synced': 1,
        }, **kwargs} for seed in range(1, 11)])
    with Pool(16, initializer=np.random.seed) as p:
        sim_list = list(tqdm(analysis.starstar(p.imap, run_synchrony, kwargs_list),
                             total=len(kwargs_list),
                             disable=False))
    ...
    for kwargs, sim in zip(kwargs_list, sim_list):
        df_data['DCN rate (Hz)'].append(sim['DCN_rate_Hz'][0])
        df_data['DCN rate (norm.)'].append(sim['DCN_rate_Hz'][0]/baseline_rates[kwargs['seed']-1])
...
def run_synchrony_manyseeds(**kwargs):
    ...
    with Pool(16, initializer=np.random.seed) as p:
        sim_list = list(tqdm(analysis.starstar(p.imap,
    ...
    baseline_rates = [np.mean([sim['DCN_rate_Hz'][0] for kwargs, sim in zip(kwargs_list, sim_list)
                      if len(kwargs['sync_indices'])==1 and kwargs['seed']==seed])
                      for seed in seeds]
    
    for kwargs, sim in zip(kwargs_list, sim_list):
        df_data['DCN rate (Hz)'].append(sim['DCN_rate_Hz'][0])
        df_data['DCN rate (norm.)'].append(sim['DCN_rate_Hz'][0]/baseline_rates[kwargs['seed']])
        df_data['two_sync'].append(
            ('none' if len(kwargs['sync_indices'])==1 else
                 ('smallest' if kwargs['sync_indices'][0]==0 else 'largest')
             )
            )
        df_data['sync percent'].append(sim['percent'])
        df_data['sync size (nS)'].append(sim['input_sizes_nS'][0])
...
def save_synchrony_xcorr(**kwargs):
    ...
    input_sizes_nS, sync_indices, N_inputs = sync_input_sizes(**kwargs)
    kwargs['input_sizes'] = input_sizes_nS*br.nS
    kwargs['fname'] = (f'{PLOTPATH}xcorr_seed_{kwargs["seed"]}'
...
def plot_synchrony_xcorr(**kwargs):
    ...
    for seed in seeds:
        ...
        fnames = glob(f"{PLOTPATH}xcorr_seed_{seed}"
                      f"_sync_indices_*.dat")
...
def save_synchrony_xcorr_twolargestsmallest(**kwargs):
    ...
    input_sizes_nS, sync_indices, N_inputs = sync_input_sizes(**kwargs)
    kwargs['input_sizes'] = input_sizes_nS*br.nS
    kwargs['fname'] = (f'{PLOTPATH}xcorr_twolargestsmallest_seed_{kwargs["seed"]}'
                       f'_sync_indices_{"_".join(map(str,sync_indices))}.dat')
    fig2.save_xcorr_parallel(**kwargs)
...
def plot_synchrony_xcorr_twolargestsmallest(**kwargs):
    ...
    for seed in seeds:
        ...
        fnames = glob(f"{PLOTPATH}xcorr_twolargestsmallest_seed_{seed}"
                      f"_sync_indices_*.dat")
        ...
        for fname in fnames:
            ...
            df_data['sync percent'].append(100*p)
            df_data['two_sync'].append(
                ('none' if len(sync_indices)==1 else
                     ('smallest' if sync_indices[0]==0 else 'largest')
                 )
                )
            df_data['seed'].append(seed)
            df_data['spike mean (norm.)'].append(np.mean(hist)/baseline)
...
def sync_input_sizes(**kwargs):
    ...
    input_sizes_nS = np.insert(np.delete(input_sizes_nS, sync_indices), 0, sum(input_sizes_nS[sync_indices]))
    # kwargs['input_sizes'] = input_sizes_nS*br.nS
...
def run_synchrony_uniform(**kwargs):
    ...
    for N_synced in range(N_inputs):
        ...
        input_sizes_nS = [size_nS*N_synced] + [size_nS for _ in range(N_inputs-N_synced)]
        kwargs_list.append({**kwargs, 'input_sizes': input_sizes_nS*br.nS, 'N_synced': N_synced})
...
def plot_synchrony_uniform_xcorr(**kwargs):
    ...
    for fname in fnames:
        ...
        hist = hist_list[0]
        plt.plot(bin_centres*1000, hist/(hist[0] or 1), color=cmap.to_rgba(sync_size_nS))
    ...
    df_xcorr = (pd.DataFrame(df_xcorr_data)
                .sort_values(['sync size (nS)', 'time (ms)'])
                )
    df_xcorr.to_csv(f'{PLOTPATH}fig6_uniform_xcorr.csv')    
    
    df = (pd.DataFrame(df_data)
          .sort_values('sync size (nS)')
          )
