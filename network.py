...
def run_DCN_wave(**kwargs):
    """Runs a PC-DCN network with the empirical PSCs.

    The excitatory and inhibitory conductance waves are manually generated beforehand,
    by convolving the PSC shapes with the spike times.
    For this reason, long simulations (>100s) are better implemented as multiple shorter simulations.
    """
    ...
    elif target == "cython":  # works best for a small number of parallel simulations
        br.set_device("runtime")
    ...
    if "PC_indices" not in kwargs and "gI" not in kwargs:
        ...
        PC_indices, PC_times = analysis.fitted_PC_spikes(
            range(N_PC), T, mean_PC_ISI=kwargs.get("mean_PC_ISI", 1 / (80 * br.Hz))
        )
    else:
    ...
    if kwargs.get("gI", False):
        ...
        if not gI_array_loaded:
            w_list = np.array(
                kwargs.get("w", [1 * br.nS for _ in range(len(connect["i"]))])
            )
            ...
            if kwargs.get("complete_gI", False):
                ...
                for i_PC, j_DCN, w, idx in zip(
                    connect["i"], connect["j"], w_list, range(len(w_list))
                ):
                    weightedtimes_complete[
                        j_DCN, idx, PC_timesteps[PC_indices == i_PC]
                    ] += (w / br.nS)
                gI_array_complete_nS = np.array(
                    [
                        [
                            scipy.signal.convolve(
                                weightedtimes_complete[j_DCN, idx], IPSC, "full"
                            )[:N_timesteps]
        ...
        save_gI_array = kwargs.get('save_gI_array','')
        if save_gI_array:
            np.savetxt(save_gI_array, gI_array_nS)
    ...
    if kwargs.get("gE", False):
        ...
        if not gE_array_loaded:
            gE_array_nS = (
                ...
                * scipy.signal.convolve(
                    np.random.poisson(
                        N_extE_DCN * (IPSC_timestep / br.second), N_timesteps
                    ),
                    EPSC,
                    "full",
                )[:N_timesteps]
        ...
        save_gE_array = kwargs.get('save_gE_array','')
        if save_gE_array:
            np.savetxt(save_gE_array, gE_array_nS)
    ...
    DCN.v = kwargs.get("DCN_v", Vr)
    sp_DCN = br.SpikeMonitor(DCN, name="sp_DCN")
    st_DCN = br.StateMonitor(
        DCN,
        kwargs.get(
            "variables",
            [
    ...
    if "out" in kwargs:
        result = {}
        for out_key, out_value in kwargs["out"].items():
            result[out_key] = eval(out_value)
        return result
    return net
