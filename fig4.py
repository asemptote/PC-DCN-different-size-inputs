...
DF_CELLRECS = {
    name: (
        pd.read_csv(f"data/{name}.csv").assign(
            trial_avg=lambda df_x: np.mean(
                [df_x[col] for col in df_x if col.startswith("trial")], axis=0
            )
        )
    )
    for name in CELL_NAMES
}
DF_CELLRECS_FILTERED = {
    name: (
        ...
        .filter(
            ...
            + [
                col
                for col in DF_CELLRECS[name]
                if col.startswith("trial ")
                and (lambda x: list(x) == sorted(x) and len(x) == len(set(x)))(
                    DF_CELLRECS[name].sort_values("size")[col]
                )
            ]
        )
        .assign(
...
def fit_all_cells():
    ...
    for cellname in CELL_NAMES:
        ...
        if avgtrials:
            os.system(
                f"start /min python fig4.py fit_uniforminputs(cellname='{cellname}', trial=None, {argstring})"
            )
        else:
            trials = [
                int(key.split()[-1])
                for key in DF_CELLRECS[cellname].keys()
                if key != "size"
            ]
            for trial in trials[:-1]:
                os.system(
                    f"start /min python fig4.py fit_uniforminputs(cellname='{cellname}', trial={trial}, {argstring})"
                )
            os.system(
                f"start /min /wait python fig4.py fit_uniforminputs(cellname='{cellname}', trial={trials[-1]}, {argstring})"
            )
...
def plot_all_cells(**kwargs):
    ...
    for (
        ...
        re.findall(
            "fits_(.*)_trial_(.*)_(.*)_(.*)_(.*)_(.*)_(.*)_niters_(.*).gif", fname
        )[0]
        + (fname,)
        for fname in fnames
    ]:
        if (trial_str != "None" and avgtrials) or (
            trial_str == "None" and not avgtrials
        ):
            continue
        ...
        kwargs["N_extE_DCN"] = EMPIRICAL_N_extE_DCN[
            f"E{params['E conductance (nS)']}_times_A"
        ]
    ...
    for row in df_gICV.iloc:
        df.loc[
            abs(df["input size (nS)"] - row["input size (nS)"]) < 0.001, r"$g_I$ CV"
        ] = row[r"$g_I$ CV"]
    df_empirical_full = pd.concat(dfs_empirical_full)
    df_empirical_avgs = df_empirical_full.groupby(["size", "cellname"]).mean()
    df_empirical_avgs = df_empirical_avgs.merge(
        df_empirical[["size", "CV"]], "left", on="size"
    )
    ...
    plt.figure()
    sns.lineplot(
        df, x="input size (nS)", y="firing rate (Hz)", errorbar="se", color="black"
    )
    sns.lineplot(
        df_empirical_avgs,
        x="size",
...
def plot_uniforminputs(**kwargs):
    ...
    if "out" not in kwargs:
        ...
        kwargs["out"] = {
            "gI_mean": {
                "run_DCN_wave": "np.mean(gI.values)",
                "run_DCN_conductance": "np.mean(net['st_DCN'].gI)",
            }[netfn],
            "gI_std": {
                "run_DCN_wave": "np.std(gI.values)",
                "run_DCN_conductance": "np.std(net['st_DCN'].gI)",
            }[netfn],
    ...
    for out, kwargs_sim in zip(out_list, kwargs_list):
        df_data["input size (nS)"].append(
            kwargs_sim["total_conductance"] / kwargs_sim["N_inputs"] / br.nS
            if kwargs_sim["N_inputs"]
            else 0
        )
        df_data[r"$g_I$ CV"].append(out["gI_std"] / out["gI_mean"])
        df_data["firing rate (Hz)"].append(
            out["mean_DCN_rate"] / kwargs_sim["T"] / br.Hz
        )
...
def fit_constE(**kwargs):
    ...
    kwargs_list.append({**kwargs_list[0], "gE": 0, "gI": 0})
    xs = []
    parallel = kwargs["parallel"]
    ...
    with Pool(len(N_inputs_list) + 1) if parallel else analysis.yieldexpr(None) as p:
        ...
        if kwargs.get("plot", False):
            ...
            fig, ax = plt.subplots()
            losses, dfs = zip(
                *[lossfn_constE(x, p, kwargs_list, return_df=True) for x in xs]
            )
...
def lossfn_constE(x, p, kwargs_list, return_df=False):
    ...
    if kwargs["Vr"] > kwargs["theta"] or kwargs["V_L"] < kwargs["theta"]:
        return np.inf
    ...
    for size in df_cell["size"]:
        loss += abs(
            df_rates[df["input size (nS)"] == size].iloc[0]
            - df_cell_rates[df_cell["size"] == size].iloc[0]
        )
...
def fit_uniforminputs(**kwargs):
    ...
    with Pool(len(N_inputs_list)) if parallel else analysis.yieldexpr(None) as p:
        ...
        if kwargs.get("plot", False):
            ...
            fig, ax = plt.subplots()
            losses, dfs = zip(
                *[lossfn_uniforminputs(x, p, kwargs_list, return_df=True) for x in xs]
            )
            ani = animation.FuncAnimation(
                plt.gcf(),
                plot_df,
            ...
            x_str = "_".join([f"{x:.1f}" for x in x_fit])
            ani.save(
                f"{PLOTNAME}{kwargs['cellname']}_trial_{kwargs['trial']}_{x_str}_niters_{len(xs)}.gif",
                writer=writer,
            )
    return x_fit
    ...
    for k in df_cell.keys():
        if k != "size":
            plt.plot(
                df_cell["size"],
                df_cell[k],
                color="black" if k == f"trial {trial}" else "grey",
            )
...
def run_uniforminputs(**kwargs):
    try:
        ...
        if "empirical_PC" in kwargs:
            empiricalPC_spiketimes_sec = [
                x[~np.isnan(x)]
                for x in pd.read_csv("data/PC in vivo data_latest.csv").values.T
            ]
            ...
            kwargs["PC_indices"], kwargs["PC_times"] = analysis.generate_PC_spikes(
                ISI_list, br.second, range(kwargs["N_DCN"] * N_inputs), kwargs["T"]
            )
